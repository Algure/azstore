...
void main()  {
  group('Azure Tables Tests',() {
   ...
   test ('Filter table',() async {
    ...
    test('Delete tables row. Returns void',() async {
      await storage.deleteTableRow(tableName: testTableName, partitionKey: myPartitionKey, rowKey: myRowKey);
    });
  ...
  group('Azure queue tests',(){
    ...
    test('Get queue list',() async {
      ...
      bool isFine=result.length>0;
      expect(isFine, true);
    });

    test('put queue messages',() async {
      for(int i=1; i<=10; i++) await storage.putQMessage(qName:testQName, message: 'testing inputs $i');
    });

    test('peek queue message',() async {
      List<AzureQMessage> result = await storage.peekQmessages(qName: testQName, numofmessages: 10);
      for(AzureQMessage q in result)print(q);
    ...
    test('Get queue messages',() async {
      List<AzureQMessage> result = await storage.getQmessages(qName: testQName,//Required
          numOfmessages: 5,
        visibilitytimeout: 3//Optional. Number of messages to retrieve. This package returns top 20 filter results when not specified.
      );
      bool limitTest= result.any((element) =>element.messageText!.contains('6'));
    ...
    test('Delete Queue message',() async {
      await Future.delayed(const Duration(seconds: 120));
      var storage = AzureStorage.parse(connectionString);
      await storage.delQmessage(qName: testQName, messageId: messageId??'', popReceipt:popReceipt??'');
    });
